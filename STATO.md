...
1. **Bloccanti infra**: (a) ~~VPN PoliMi~~ ✅ **funzionante** (testata 21/7 su Jhonny: `gp-saml-gui --gateway` + openconnect, howto §1; hostname eagle risolve, ssh arriva al server); (b) chiave SSH — **unico bloccante rimasto**: il server chiede la password → nessuna chiave ancora in `authorized_keys`; Jimmy inviata a Thomas 19/7, Jhonny (howto §2) da mandargli insieme; attesa abilitazione, poi `ssh multispectralwaste.eagle` entra; (c) ~~username GitLab a Enrico~~ → **msg a Enrico inviato (19/7)**, attesa risposta per call mar/mer.
