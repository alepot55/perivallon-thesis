...
| C-2 | **Riformulato**: lungo il GSD degrada la LOCALIZZAZIONE, non la detection — e non linearmente | **SUPPORTED** | Curva a 3 livelli 30/60/120 cm nella pipeline gruppo (EXP-015/017/019, 60 cm derivato da noi): detection piatta 0.71-0.76; pointing game 0.38 → 0.30 → 0.04, cioè −0.08 al primo raddoppio di GSD e −0.26 al secondo (ginocchio fra 60 e 120 cm). Figura: `figs/gsd_detection_vs_localization.png` | Cap. risultati, fig. chiave |
| C-3 | A 1.2 m (risoluzione IRIDE-like) la detection resta operativamente utilizzabile, la localizzazione no | **SUPPORTED (risposta articolata)** | EXP-019: a 120 cm F1 ~0.70 (come a 30 cm) ma pointing game 0.04-0.06 → per ARPA il sistema a 1 m dice *se* guardare un sito, non *dove* intervenire; il ginocchio cade proprio a ridosso di IRIDE. Soglia operativa da discutere con Thomas | Discussione/impatto |
...
| C-6 | Un metodo oltre vanilla Grad-CAM (distillazione cross-GSD delle CAM) migliora la localizzazione a bassa risoluzione in modo misurabile | OPEN — **evidenza a favore + controllo superato** | EXP-018 (λ=1, seed 0, @120cm): mean best IoU 0.075→0.155, MaxBoxAcc 0.02→0.08, detection invariata (−1 pp); pointing game piatto (conteggi rari su 50 img). **EXP-020**: l'ablation simmetrica SSENet-style peggiora (IoU 0.046) → il guadagno viene dall'informazione HR del teacher, non dal vincolo in sé. Restano: sweep λ con selezione su val (in corso) + multi-seed | Contributo principale (+2 punti) |
...
- 2026-07-24 (notte, 4): C-2 riformulato e SUPPORTED (curva 30/60/120: degrada la localizzazione, non la detection, con ginocchio fra 60 e 120 cm); C-3 SUPPORTED con risposta articolata; C-6 rafforzato dall'ablation simmetrica fallita (EXP-020).
