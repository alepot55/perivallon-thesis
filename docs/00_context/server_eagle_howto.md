...
- **Linux nativo — via consigliata (script gestito)**: `sudo apt install openconnect gp-saml-gui` (su Ubuntu 24.04 è pacchettizzato; NON è su PyPI), poi **`bash docs/00_context/vpn_eagle.sh up`**. Al primo avvio apre la finestra di login SAML PoliMi (SPID); il cookie di sessione che ne esce **vale ~30 giorni** e viene salvato in `/root/.vpn-eagle-cookie`, quindi gli `up` successivi si connettono da soli, senza SPID. openconnect gira come unità systemd `vpn-eagle` (sopravvive alla chiusura del terminale); `down` spegne pulito (DNS ripristinato), `status` dice come stai. Setup fatto su Jhonny il 21/7.
- **Linux nativo — via manuale** (se non usi lo script): `gp-saml-gui --gateway gp-deib-saml.vpn.polimi.it` apre il login e a fine flusso **stampa** il comando `echo <cookie> | sudo openconnect --protocol=gp ... --usergroup=gateway:prelogin-cookie ...` da incollare subito (cookie monouso, scade in fretta). Il flag `--gateway` è necessario: senza, openconnect consuma il cookie sul portal e si blocca su `prelogin-cookie: fgets (stdin)` (verificato 19/7). La VPN resta su finché quel terminale è aperto. Nota: se openconnect muore ucciso (non con Ctrl+C) può lasciare il DNS rotto — fix: `sudo systemctl restart systemd-resolved` (se non basta: cancella `/run/systemd/resolve/stub-resolv.conf` e riavvia di nuovo il servizio).
