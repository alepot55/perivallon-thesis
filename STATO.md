...
3. **Claude**: (1) ~~doc-baseline Gibellini~~ ✅ 21/7; (2) ~~mini-SOTA WSOL~~ ✅ 21/7 (verdetto sopra); (3) ~~indice tesi v0~~ ✅ 21/7 (`docs/04_planning/indice_tesi_v0.md`); (4) related work detection (riorienta `docs/02_research/loop_prof_sota/10_related_work_draft.md`); (5) tenere vivi `EXPERIMENTS_LOG.md` + `CLAIMS.md`.
4. ~~Prenotazione GPU + sanity-training~~ ✅ 21/7 sera: EXP-001 completato (val F1 0.729 @0.3m vs 0.688 @1.2m, v. `EXPERIMENTS_LOG.md`). **Baseline seria programmata**: slot mer 22/7 10-12 GPU 1 (foglio Turni "Waste (Ale)"), parte da sola alle 10:00 (tmux `baseline` su eagle) — Swin-T+RSP two-step Gibellini, batch 120, norm per banda, 0.3m poi 1.2m; smoke-test pesi/modello passato la sera prima. Script mirror in `eagle/`.
