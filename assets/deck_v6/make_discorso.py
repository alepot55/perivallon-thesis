...
(2,"The problem: priority, not just presence",
 "Lo smaltimento illegale di rifiuti — le discariche abusive — è un crimine ambientale e un problema di salute pubblica. ARPA non può ispezionare tutto: deve <b>dare priorità</b>, e la priorità dipende da <i>cosa</i> è stato sversato — il materiale. Le pipeline automatiche oggi trovano bene i siti, ma classificano la presenza, non il contenuto. Lo dice la survey dello stesso professore: su 50 lavori, quasi tutti RGB, e l'identificazione del materiale è dichiarata gap aperto — servono ≤30 cm e dati multispettrali. Da qui la domanda di ricerca, aperta: qual è il valore aggiunto del multispettrale, rispetto al solo RGB, per classificare il materiale?",
...
(3,"Why material matters: risk = hazard × exposure × magnitude",
 "Il pericolo dipende dal materiale: macerie inerti, plastiche e cemento-amianto sono mondi diversi. È il materiale a portare il <b>pericolo</b> — il Catalogo Europeo dei Rifiuti (EWC) marca i codici pericolosi con l'asterisco: l'amianto è 17 06 05*. Il rischio prioritario è <b>pericolo × esposizione × magnitudine</b>. E non è teoria: Regione Lombardia lo operazionalizza già per i tetti in amianto con l'Indice di Degrado — la soglia del punteggio decide tra rivalutazione, bonifica in 3 anni o rimozione in 12 mesi. Quindi identificare il materiale — non solo localizzare il sito — è la variabile decisionale.",
...
(7,"From bands to information: three views of the same scene",
 "Stessa immagine multibanda, tre letture diverse: l'RGB — quello che vediamo noi; il falso colore infrarosso — la vegetazione diventa rossa; e l'NDVI — un indice calcolato dalle bande. Il punto: le bande sono <b>informazione</b>, e combinazioni diverse rispondono a domande diverse. Per discriminare i materiali serve più dell'RGB.",
...
(9,"RGB fails in two distinct ways",
 "L'RGB fallisce in due modi distinti. Qui il primo: l'<b>iso-cromaticità</b> — materiali diversi, stesso colore. Nel grafico, cemento e tetto in cemento-amianto: nel visibile quasi indistinguibili (ΔR ≈ 0,11, entrambi grigi — le foto a destra lo mostrano), ma nello SWIR solo il tetto in amianto ha il dip Mg-OH a 2,31 µm. Senza spettro non si separano.",
 "Primo fail: iso-cromaticità (ΔR ≈ 0,11 nel visibile; Mg-OH 2,31 µm nello SWIR).",
...
(13,"A moving field: material-level evidence is 2024-2026",
 "Questa timeline colloca i lavori per anno e risoluzione. Si vede la storia in un colpo d'occhio: il rilevamento dei <i>siti</i> in RGB è maturato nel 2023-2024 — AerialWaste, Sun, CascadeDumpNet, il servizio Disaitek. La discriminazione del <i>materiale</i> sta succedendo <b>adesso</b>, 2024-2026: Aguilar sullo SWIR, Shepherd ed EMIT dall'iperspettrale, Saba e Bonifazi su WorldView-3. Ma nessuno di questi è dentro una pipeline di rifiuti illegali alla risoluzione del task — la fascia grigia in basso è ciò che è troppo grossolano (>5 m). Lo stato dell'arte è vivo, e il buco è esattamente dove si colloca la tesi.",
...
(14,"More bands ≠ more information, well-chosen few suffice",
 "Non conta quante bande, conta <b>quali</b>. Vitek 2025, rifiuti da costruzione: RGB a 0,87 sale a 0,96 con sole 2 bande NIR — alla pari con 768 bande iperspettrali. SpectralWaste 2024: la fusione RGB+SWIR batte entrambi da soli, e lo SWIR vince proprio sulle classi sottili e iso-colore. L'ancora storica è Aguilar 2021: il salto da 90,85 a 97,38 veniva dallo SWIR. <b>Il perché conta</b>: quali bande servono dipende dal materiale — per alcuni il VNIR basta (Saba 2026), per i pericoli legati alla chimica servono le bande SWIR. È esattamente ciò che un'ablazione controllata deve misurare.",
...
(16,"Very-high resolution: the SWIR divide",
 "All'altissima risoluzione c'è uno spartiacque: chi porta lo SWIR. WorldView-3 è l'unica piattaforma sotto i 2 m che lo porta — dove sta la chimica. Pléiades Neo e SuperDove sono solo-VNIR: nitidi ma ciechi alle assorbanze diagnostiche. Precisione importante (la trappola Sentinel-2): S-2 lo SWIR <b>ce l'ha</b>, a 20 m — lì il limite è la risoluzione, non la banda. La domanda VHR è diversa: chi porta lo SWIR sotto i 2 metri. Nota: qui i sensori sono il <b>panorama</b> del task, non un dataset che possiedo.",
...
QA=[
("\"Ma esattamente che task è?\"","Classificazione del <b>materiale</b> del rifiuto da satellite, per stimare il rischio. Il rilevamento del sito è sostanzialmente risolto in RGB (Gibellini F1 92%); il contributo aperto è il materiale. Nota: ogni tabella-survey ha ora la colonna <b>Task</b> (classification / object detection / segmentation / target detection) — saperla leggere per ogni lavoro citato: Gibellini = classificazione a livello immagine; Sun e CascadeDumpNet = object detection; MARIDA e CWLD = segmentazione; U-Net = segmentazione (encoder-decoder)."),
...
for num,tit,disc,rem,tr in sorted(S, key=lambda x: x[0]):
