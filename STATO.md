...
1. **Bloccanti infra**: (a) ~~VPN PoliMi~~ ✅ **funzionante** (testata 21/7 su Jhonny: `gp-saml-gui --gateway` + openconnect, howto §1; hostname eagle risolve); (b) chiave SSH — **Jimmy inserita da Thomas 21/7** ✅ (da Jimmy l'accesso dovrebbe funzionare); Jhonny **non ancora abilitata**: in chat era stata incollata due volte la chiave di Jimmy per errore (verificato 21/7 con `ssh -v`: server rifiuta le chiavi di Jhonny) → abilitare self-service da Jimmy (one-liner in howto §2) o mandare a Thomas la chiave giusta; (c) ~~username GitLab a Enrico~~ → **msg a Enrico inviato (19/7)**, attesa risposta per call mar/mer.
