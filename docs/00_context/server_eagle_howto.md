...
- **Linux nativo**: (a) prova il portale da browser: dopo il login la pagina Palo Alto a volte offre il pacchetto Linux del client ufficiale; (b) altrimenti open-source: `sudo apt install openconnect gp-saml-gui` (su Ubuntu 24.04 è pacchettizzato; NON è su PyPI), poi `gp-saml-gui gp-deib-saml.vpn.polimi.it` — apre la finestra di login SAML PoliMi. **Attenzione**: questa versione NON attacca la VPN da sola — a login riuscito **stampa** il comando `echo <cookie> | sudo openconnect --protocol=gp ... --usergroup=portal:prelogin-cookie ...` che va incollato subito (il prelogin-cookie è monouso e scade in fretta: se "authentication failed", rifare gp-saml-gui; se fallisce ancora, provare `--usergroup=gateway:prelogin-cookie`). Verificato funzionante il 19/7 (login SAML anche via SPID). La VPN resta su finché il terminale con openconnect è aperto (Ctrl+C per staccare) → ssh da un secondo terminale.
