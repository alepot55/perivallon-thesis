...
| 020 | Serve davvero il maestro a 30 cm, o basta la consistenza fra scale (SSENet)? | Simmetrica: IoU **0.046** (peggio della baseline 0.075); col maestro: 0.155 | **L'asimmetria è il punto**: riscalare un'immagine a 120 cm non crea dettaglio, il maestro invece porta informazione vera. È la prova che ci distingue da SSENet/SEAM |
| 019 | **La curva completa** (aggiunto il 60 cm che abbiamo derivato noi) | detection piatta 0.71-0.76 ovunque; pointing game 0.38 → 0.30 → 0.04 | **Il ginocchio è tra 60 e 120 cm**: il primo raddoppio di GSD costa 0.08 di localizzazione, il secondo 0.26 (3×). A 60 cm si localizza ancora, a 120 no — e IRIDE sta lì. Figura chiave: `figs/gsd_detection_vs_localization.png` |
