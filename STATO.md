...
Pivot del 17/7: binary landfill detection satellite-only, asse **risoluzione** (ufficiale dalla call 24/7: **120 cm pronto / 30 cm ~settimana prossima / 60 cm lo deriviamo noi**). Obiettivo: **≥7 punti a dicembre**; North Star: *weakly-supervised waste localization under GSD degradation* — settimana 21-24/7: EXP-001→010, protocollo WSOL + tetto geometrico + retrain 448 (localizzazione ×7) + fenomeno "la risoluzione colpisce il dove, non il se". **Fase attuale (post-call 24/7): allineamento alla pipeline del gruppo** — branch GitLab "Alessandro Potenza" (link in arrivo da Enrico), Excel "Binary Experiments Results" da compilare, esperimenti ufficiali partono dal 120 cm. Call: `docs/01_calls/2026-07-24_call_enrico_thomas_codice.md`.
...
> ⚠️ **Manutenzione server eagle: lunedì 27/7 ore 10:00** (~20 min, riavvio, tmux persi — mail Luca 24/7). Nessun job a cavallo delle 10; dopo, ricontrollare ssh/VPN e ricreare eventuali tmux.

## ✅ TODO aperti

1. **Attese esterne**: (a) ~~link GitLab~~ → repo trovata 24/7 sera: **`gitlab.com/wds-co/multispectralpotenza`** (account Ale `alessandro-potenza`; token "claude" scad. 2027-02-11, salvato SOLO nel credential store su eagle `~/.git-credentials`). **Blocco: Ale è Guest (liv. 10), serve Developer (30) per clone+push** — chiesto upgrade a Enrico; sentinella API attiva che avvisa Claude appena sbloccato; (b) dataset 30 cm (Thomas avvisa, ~settimana prossima); (c) chiarire **VNIR vs FS** nell'Excel (probabile prima domanda a Enrico).
2. **Claude, in ordine post-call**: (1) ~~esplorare il codice di Enrico~~ ✅ 24/7 sera — accesso Developer ottenuto, clone su eagle `/home/dev/multispectralpotenza` + copia lettura locale `~/Desktop/uni/multispectralpotenza`; mappa completa in `docs/02_research/2026-07-24_mappa_codice_enrico.md` (tile 8-bit pre-tagliate su /scratch, tl→ft Adam early-stop, CAM multiple, SwinT solo-RGB da estendere, 6 domande aperte per Enrico); (2) ~~esperimenti chiave nella loro pipeline a 120 cm~~ ✅ 24/7 sera, EXP-011→014: resnet50-RGB 0.635 · **SwinT-RGB 0.694±0.019 (3 seed)** · SwinT-6bande 0.665±0.006 — SwinT +7 pp su resnet50; **a 120 cm lo spettro non compensa (C-4, evidenza su due pipeline)**; SwinT multibanda implementata nel loro codice (branch `ale` su GitLab, con fix vari); righe pronte per l'Excel in `docs/04_planning/excel_rows.md` (Ale le incolla — file Excel da farsi condividere); prossimo giro: coppia RGB/6-bande sul **30 cm** appena esce, poi curva 30→60→120; (3) ~~script derivazione 60 cm~~ ✅ 24/7 — `eagle/derive_60cm.py` (mirror + server `~/experiments/`), block mean 2×2 testato su finestra reale di MS_pansharpened (700→350 px, 6 bande, bounds identici, diff 0.0; i 5 mosaici 30 cm sono GIÀ su archive). Nota letteratura: lo standard per simulare GSD più grosso è filtro gaussiano MTF-matched + decimazione (protocollo Wald); la media 2×2 è la variante boxcar che Thomas ha indicato ("media o massimo") — da confermare con lui; (4) migrare la valutazione WSOL nel loro flusso (evaluation notebook ha già visualize-CAM); (5) ~~skill automation-recommender~~ ✅ 24/7 — hook `.claude/hooks/repo_guard.py` (blocco file auto-generati papers/ + reminder checklist post-EXPERIMENTS_LOG), `/eagle` (stato server in un colpo), `/experiment` aggiornato a naming di gruppo + checklist Excel/GPU/guida.
3. **Regole operative nuove (vincolanti)**: prenotazioni GPU al minimo indispensabile e togliersi appena finito; nomi esperimenti descrittivi (`b120_vnir_swint_rsp_aug1_s42`); una modifica alla volta; Excel Drive aggiornato a ogni run.
