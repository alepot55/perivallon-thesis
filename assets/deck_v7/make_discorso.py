# -*- coding: utf-8 -*-
"""Discorso per deck v7 (25 slide). HTML -> PDF via wkhtmltopdf."""
...
(15,"Asbestos deep-dive",
 ...
 "Per completezza, anche gli oggetti a forma fissa."),
(16,"Objects and close range",
 "Per completezza, il resto della letteratura in scope: oggetti a forma fissa e distanza ravvicinata. Cisterne: detection matura, precision 96 e oltre 169 mila serbatoi mappati. Container e veicoli: detection consolidata a 30-50 centimetri. Rottami: solo close-range con infrarosso. Da drone: segmentazione di rifiuti generici e volumi di macerie con fotogrammetria. Il pattern è chiaro: gli oggetti definiti dalla forma sono maturi; la composizione del materiale richiede ancora spettro o distanza ravvicinata.",
 "Pattern: forma = risolto, composizione = aperto. Non dilungarsi, è una slide di completezza.",
...
(24,"References 1/2",
 ...
 "E la seconda pagina."),
(25,"References 2/2",
 "E i lavori su oggetti, close range e la libreria spettrale USGS. In tutto 23 lavori citati dalla libreria di 47; il resto è tenuto come contesto vagliato. Grazie, e sono pronto per le domande.",
 "23 citati / 47 in libreria / 699 record iniziali: la catena dei numeri regge.",
 "— Domande."),
]
...
TIPS=[
"25 slide, ~20 minuti. Presentazione in italiano, termini tecnici in inglese.",
"Flusso: contesto → task (definizione + schema) → materiali (tassonomia, in/out, copertura) → ricerca (metodo, esclusi) → siti (tabella + 3 deep-dive) → materiali (tabella + amianto + oggetti) → limiti RGB + dati → sintesi → gap → proposta (approccio, pilot, valutazione) → riferimenti.",
"Le slide di deep-dive (11-13, 15) sono il cuore nuovo: dimostrano che i lavori chiave sono letti a fondo, non solo tabulati.",
"Slide 16 e 19 sono di completezza/sintesi: passarci veloci, non rileggerle riga per riga.",
...
parts.append("<h1>Discorso — deck v7 (25 slide)</h1>")
